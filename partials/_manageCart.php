...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE vadapavId = '$itemId' AND `userId`='$userId'";
       $result = mysqli_query($conn, $existSql);
        $numExistRows = mysqli_num_rows($result);
        //$result = isset($conn, $existSql);
        //$numExistRows = isset($result);    
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`vadapavId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())"; 
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `vadapavId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    $vadapavId = $addrow['vadapavId'];
                    $itemQuantity = $addrow['itemQuantity'];
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `vadapavId`, `itemQuantity`) VALUES ('$orderId', '$vadapavId', '$itemQuantity')";
                ...
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                    window.location.href="http://localhost/VilliansAdda/index.php";  
                    </script>';
                    exit();
            }
        } 
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        ...
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `vadapavId`='$vadapavId' AND `userId`='$userId'";

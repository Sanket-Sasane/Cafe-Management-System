...
<body>
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `menu` WHERE MATCH(vadapavName, vadaDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ?><script> document.getElementById("items").(innerHTML = "Items: ");</script> <?php
                $noResult = false;
                $vadapavId = isset($row['vadapavId']);
                $vadapavName = isset($row['vadapavName']);
                $vadaPrice = isset($row['vadaPrice']);
                $vadaDesc =isset( $row['vadaDesc']);
                $vadaCategorieId = isset($row['vadaCategorieId']);
                
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/pizza-'.$vadapavId. '.jpg" class="card-img-top" alt="image for this pizza" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title">' . substr($vadapavName, 0, 20). '...</h5>
                            <h6 style="color: #ff0000">Rs. '.$vadaPrice. '/-</h6>
                            <p class="card-text">' . substr($vadaDesc, 0, 29). '...</p>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE vadapavId = '$vadapavId' AND `userId`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$vadapavId. '">
                                ...
                                echo '</form>
                                <a href="viewPizza.php?vadapavId=' . $vadapavId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';
